...
-- name: ListPurchaseSpendSeries :many
...
-- name: ListTopSuppliersBySpend :many
WITH active_purchase_lines AS (
    SELECT
        document.id AS document_id,
        document.counterparty_id,
...
-- name: ListFreeStockEntrySeries :many
...
-- name: ListProductionByRecipeProduct :many
...
-- name: ListProductionDirectCostSeries :many
WITH active_production_runs AS (
    SELECT
        document.id AS document_id,
        CAST(
...
-- name: ListProductionYieldVariance :many
WITH active_production_runs AS (
    SELECT
        document.id AS document_id,
        recipe.id AS recipe_id,
